...
function BasketCart({ basketRef }) {
    ...
    return (
        <div className='absolute z-50 h-fit  top-12 w-[84%] md:w-1/3 right-16 md:right-8 rounded-md divide-y-2   bg-teal-800 p-2' ref={basketRef}>
            {selectedProducts.map((product, index) => (
                <div key={index} className='flex items-center text-white justify-between  p-2  border-gray-200'>
            ...
            <Link className=' mx-auto block m-2 text-center p-1.5 hover:bg-teal-500 ease-in-out transition-all hover:text-white bg-white rounded-lg text-teal-700 '
             to="/checkout">
            CheckOut
            </Link>
        </div>
